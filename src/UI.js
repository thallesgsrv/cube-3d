...
export class UI {
	...
	_bind() {
		...
		document.querySelector('.js-scramble').addEventListener('click', () => { this.toast.classList.remove('is-visible'); const axes = ['x', 'y', 'z']; const layers = [-(this.cube.size - 1) / 2, (this.cube.size - 1) / 2]; for (let i = 0; i < 20; i += 1) this.cube.applyMove({ axis: axes[i % 3], layerValue: layers[i % 2], dir: i % 2 ? -1 : 1 }); this.renderer._updateCube(); this.renderer._triggerMoveEffect(); this.refresh(); });
